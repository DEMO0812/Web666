	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		auth.userDetailsService(username -> {
			...
			try {
				...
				System.out.println("tc1" + username);
				...
				System.out.println("t3" + roles);
				return User.withUsername(username).password(password).roles(roles).build();

			} catch (Exception e) {
				System.out.println("tb" + username);
...
//	                    .map(authority -> authority.getRole().getRoleId())
//	                    .toArray(String[]::new); // Sử dụng method reference
//	            System.out.println("t3" + Arrays.toString(roles)); // In ra mảng roles để debug
//	            return User.withUsername(username).password(password).roles(roles).build();
//	        } catch (Exception e) {
//	            System.out.println("tb" + username);
//	            throw new UsernameNotFoundException(username + " not found! / securityConfig", e);
//	        }
//	    });
//	}
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable();
		http.authorizeRequests().antMatchers("/order/**").authenticated()// khách hàng
		...
		http.exceptionHandling().accessDeniedPage("/security/unauthoried");// đang nhập rồi và truy cập trang ko đk cấp
																			// quyền
